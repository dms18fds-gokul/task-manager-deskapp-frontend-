...
import { Settings, Users, AlertCircle, Search, Lock, X } from 'lucide-react';
...
const EmployeeActiveControlPage = () => {
    ...
    const handleToggleActive = async (employeeId, currentStatus) => {
        if (currentStatus === true) {
            // Trying to deactivate - require password
            setSelectedEmployee(employeeId);
            setAdminPassword('');
            setPasswordError('');
            setIsPasswordModalOpen(true);
        } else {
            // Trying to activate - skip password
            await executeToggle(employeeId, false, null);
        }
    };
    ...
    const handleConfirmDeactivation = async (e) => {
        ...
        await executeToggle(selectedEmployee, true, adminPassword);
    };

    const executeToggle = async (employeeId, currentStatus, password) => {
        if (password) setIsProcessing(true);
        setPasswordError('');
        ...
        try {
            ...
            if (err.response?.status === 401 || err.response?.status === 400 || err.response?.status === 403) {
                ...
                if (password) setPasswordError("Failed to update status. Please try again.");
                else alert("Failed to activate. Please try again.");
            }
        } finally {
    ...
    return (
        <div className="flex h-screen bg-slate-50 font-sans relative">
            ...
            <div className="flex-1 overflow-x-hidden overflow-y-auto w-full max-w-full flex flex-col">
                {/* Mobile Header */}
                ...
                {/* Desktop Header Content (Keep original design but wrap in div) */}
                            ...
                            <form onSubmit={handleConfirmDeactivation}>
                                ...
                                <div className="mt-8 flex justify-end gap-3">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={isProcessing}
                                        className="px-5 py-2.5 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 disabled:bg-red-400 rounded-lg shadow-sm shadow-red-200 transition-colors flex items-center justify-center min-w-[120px]"
                                    >
                                        {isProcessing ? (
