...
const EmployeeLogsAndTaskControlPage = () => {
    ...
    const [selectedToggleType, setSelectedToggleType] = useState(null); // 'qt' or 'task'
    ...
    const handleConfirmDeactivation = async (e) => {
        ...
        const currentStatus = selectedToggleType === 'qt' ? emp?.qtActivity : emp?.taskActivity;

        await executeToggle(selectedEmployeeId, selectedToggleType, currentStatus ?? true, adminPassword);
    };
        ...
        try {
            ...
            setEmployees(prev => prev.map(emp => {
                if (emp.employeeId === employeeId) {
                    return { ...emp, [type === 'qt' ? 'qtActivity' : 'taskActivity']: !currentStatus };
                }
                return emp;
            }));
            ...
            <div className="flex-1 overflow-x-hidden overflow-y-auto w-full flex flex-col">
                ...
                <header className="bg-white border-b border-slate-200 p-4 flex justify-between items-center md:hidden z-10 sticky top-0">
                    <div className="flex items-center gap-2 text-indigo-600">
                        ...
                        <h1 className="text-lg font-bold text-slate-800 uppercase tracking-tight">Logs Control</h1>
                    </div>
                    <button
                ...
                <div className="bg-white border-b border-slate-200 px-8 py-6 shrink-0">
                    <div className="flex flex-col gap-1 max-w-7xl mx-auto">
                        ...
                        <p className="text-slate-500 text-sm mt-1">
                            Review employee activity logs and manage individual feature access
                        </p>
                    </div>
                </div>
                                            ...
                                            <tr key={emp.employeeId} className="hover:bg-slate-50/80 transition-colors">
                                                ...
                                                <td className="px-6 py-4 align-middle text-center">
                                                    <div className="flex justify-center w-full">
                                                        <ThemeToggle
                                                            isActive={emp.qtActivity ?? true}
                                                            onClick={() => handleToggleActivity(emp.employeeId, 'qt', emp.qtActivity ?? true)}
                                                            title={emp.qtActivity !== false ? "QT Active" : "QT Inactive"}
                                                        />
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 align-middle text-center">
                                                    <div className="flex justify-center w-full">
                                                        <ThemeToggle
                                                            isActive={emp.taskActivity ?? true}
                                                            onClick={() => handleToggleActivity(emp.employeeId, 'task', emp.taskActivity ?? true)}
                                                            title={emp.taskActivity !== false ? "Task Active" : "Task Inactive"}
                                                        />
                                                    </div>
                                                </td>
                                            </tr>
...
export default EmployeeLogsAndTaskControlPage;
